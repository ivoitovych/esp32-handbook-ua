...
// ── Сторінки для нотаток ────────────────────────────────────────────────
// Книгу доводиться доганяти до кратності 16: друкарня складає аркуш із
// шістнадцяти сторінок, і зайві однаково будуть — питання лише, що на
// них надрукують.
//
// Порожні — марнування. Польовий довідник лежить на столі поруч із
// платою, і людині є що записати: розпіновку свого виробу, адресу на
// шині, номер партії. Тож остача аркуша віддається читачеві.
#let notatky(skilky) = {
  for _ in range(skilky) {
    // `weak: true` — інакше два розриви поспіль дають порожній розворот:
    // кожна нотаткова сторінка виходила з порожньою парою.
    pagebreak(weak: true)
    text(font: font-sans, size: 0.8em, weight: 600, tracking: 0.06em,
         fill: luma(45%), upper("Нотатки"))
    v(0.6em)
    // Інтервал між блоками треба гасити явно: типовий `par.spacing`
    // додається до кожного `v()`, і двадцять шість лінійок переповнювали
    // сторінку рівно настільки, щоб забрати другу.
    set par(spacing: 0pt)
    for _ in range(24) {
      line(length: 100%, stroke: 0.35pt + luma(78%))
      v(1.15em)
    }
  }
}
...
// ── Предметний покажчик ─────────────────────────────────────────────────
// Дві колонки й дрібніший кегль — так набирають покажчики, і не задля
// краси: покажчик читають не рядками, а сканують очима згори вниз, тож
// вузька колонка тут швидша за широку. Заразом він перестає з'їдати
// чотирнадцять сторінок.
#let pokazhchyk(body) = {
...
// ── Титульна сторінка ───────────────────────────────────────────────────
// ── Обкладинка ──────────────────────────────────────────────────────────
// Перше, що бачить рука, а не око: назва, автор, рік. Решта — на титулі.
//
// Графіка навмисно одна й проста: сітка отворів із кроком 2.54 мм, тобто
// макетна плата в натуральну величину. Вона впізнається тим, кому книга
// призначена, друкується однією фарбою й не потребує жодного зовнішнього
// файлу — а отже, не може загубитися разом із ним.
// 46 кроків × 2.54 мм = 116.8 мм — рівно ширина набору A5 при
// полях 15 мм. Сітка мусить доходити до обох країв, інакше вона
// читається як випадковість, а не як задум.
#let sitka-platy(cols: 46, rows: 7, krok: 2.54mm) = {
  box(width: cols * krok, height: rows * krok)[
    #for r in range(rows) {
      for c in range(cols) {
        place(dx: c * krok, dy: r * krok,
              circle(radius: 0.55mm, fill: luma(45%), stroke: none))
      }
    }
  ]
}
...
#let cover(meta) = {
  set page(
    paper: "a5",
    margin: (left: 15mm, right: 15mm, top: 17mm, bottom: 15mm),
    header: none, footer: none, numbering: none,
  )
  set align(left)
  // Обкладинка — не абзац тексту: вирівнювання за шириною тут розтягує
  // заголовок на два слова через усю сторінку.
  ...
  text(font: font-sans, size: 2.55em, weight: 800, tracking: -0.02em,
  ...
  block(width: 92%, text(font: font-serif, size: 1.05em, style: "italic",
                         fill: luma(25%), meta.subtitle))

  v(1.4fr)

  line(length: 100%, stroke: 0.6pt + luma(60%))
  v(0.75em)
  text(font: font-sans, size: 1.2em, weight: 600, tracking: 0.03em,
       meta.author)
  h(1fr)
  text(font: font-sans, size: 1.1em, fill: luma(40%), meta.year)
...
#let colophon-front(meta) = {
  ...
  [Довідник дозволено *вільно друкувати, копіювати і роздавати*, зокрема
   ...
   проєкту.]
  v(1.2em)
  [*Що це за книга.* Не джерело істини першої інстанції, а *зібрання*:
   відомості з великої кількості відкритих джерел — документації Espressif,
   вихідних текстів ESP-IDF і esptool, специфікацій виробників мікросхем —
   зведені докупи, впорядковані й звірені з першоджерелами. Книга не
   заміняє документацію Espressif; вона дає швидку робочу базу тому, хто
   відкриває для себе вбудовану електроніку.]
  v(0.6em)
  [*Книга публічна.* Текст, приклади коду й *увесь реєстр фактчекінгу* —
   з дослівними цитатами, адресами джерел і станом перевірки кожного
   твердження — відкриті й доступні за адресою #meta.contact. Перевірити
   будь-яке число з цих сторінок можна не питаючи автора.]
  v(0.6em)
  [*Як зроблено.* Книгу складено приблизно за 48 годин, і *справжнього
   рецензента вона не мала*: жодна людина не прочитала всі сторінки
   поспіль перед друком. Замість людської вичитки застосовано
   автоматизовану перевірку. Понад 7800 тверджень виділено в окремі
   одиниці з власними ідентифікаторами; кожна має запис у реєстрі й клас
   перевірки — від «першоджерело отримано, цитату наведено дослівно» до
   «ще не звірено». Три незалежні автоматичні звірки стежать, щоб
   твердження не лишалося без запису, щоб цитата підпирала саме те, що
   нею підпирають, і щоб кожна цитата дослівно стояла за названою
   адресою. Докладно — у вступному розділі.]
  v(0.6em)
  [*Помилки тут є.* Автоматика зменшує їхню кількість, але не доводить до
   нуля. Числа, від яких залежить ціле, звіряйте з першоджерелом.]
  v(0.6em)
  [*Перелік помічених помилок* ведеться відкрито — файл `ERRATA.md` за
   адресою вище, з прив'язкою до ревізії, надрукованої на цій сторінці.
   Перед тим як покластися на число, від якого залежить робота, варто
   туди зазирнути. Знайдену помилку є куди надіслати, і вона потрапить
   у той самий реєстр, що й решта тверджень.]
