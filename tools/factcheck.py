...
# `E` — «поза зовнішньою звіркою». Це записане рішення, а не пропуск.
#
        ...
        for f in sorted((ROOT / g).glob("*.md")):
            ...
            for k, (vyd, txt, ln) in enumerate(odynyci, 1):
                ...
                if vyd == "kod":
                    ...
                    klas = z.get("klas", "F")
                elif vyd == "proza" and not RE_ZOVNISHNIY_SYGNAL.search(txt):
                    # Проза без жодного зовнішнього сигналу — редакційна.
                    # Клас E, і це рішення, а не пропуск (див. коментар
                    # біля RE_ZOVNISHNIY_SYGNAL).
                    klas = "E"
                else:
                    klas = "F"
