...
traceable to the thing that produced it in one step.
    ...
    tools/report.py            write factcheck/REPORT.md
    tools/report.py --show     print it without writing
    tools/report.py --demo    demonstration on a broken input
"""
...
def assemble() -> str:
    ...
    a("")
    a("> **generated** — `tools/report.py`. Editing it by hand only moves the")
    ...
    a("caught them are in `data/book/REFUTED.md`.")
    a("")
    a("## Sources")
    a("")
    a("```")
    ...
    a("become a hand-off in `data/book/UNREACHABLE-SOURCES.md`, with the")
