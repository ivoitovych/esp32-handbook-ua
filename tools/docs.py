...
def perevirka() -> list[str]:
    ...
    for imya in KERIVNI:
        ...
        # Не літерал: власна копія переліку класів у перевірці, що
        # стереже копії переліку класів. Рядок "ABCDEFGKLS" не мав
        # `N`, тож перевірка маскувала його ДО порівняння — і не
        # побачила б виправлення, якого сама вимагала.
        nazvani &= klasy_kodu() | {'K'}
