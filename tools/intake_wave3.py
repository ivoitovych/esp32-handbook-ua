...
## Чим це відрізняється від `intake.py`
...
def normal(s: str) -> str:
    ...
    s = s.replace("­", "").replace("‑", "-")
    s = re.sub(r"[–—−]", "-", s)
    # Лапки прибираються з порівняння ЗОВСІМ — знахідка М1 від 11:40Z.
    # Вони розмітка, а не зміст: у документі стоїть
    # `FreeRTOS Timer Task ("Tmr Svc")`, помічник виписує
    # `FreeRTOS Timer Task (Tmr Svc)` — і чесний витяг гине.
    # У М1 після цього частка вцілілих піднялася з 38 % до 45 %,
    # тобто сім відсотків «браку» були виною воріт, а не помічників.
    #
    # Слова, числа й регістр лишаються як є: `serial clock bus (SCL)`
    # проти `serial clock line (SCL)` — справжня хиба, і гинути мусить.
    s = re.sub(r"[’‘`´\"“”«»„‟]", "", s)
