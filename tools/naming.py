...
a recorded baseline, `factcheck/data/reports/TRANSLITERATION.md`. The check compares
...
is the only moment it is cheap to fix.
    ...
    tools/naming.py            check against the baseline
    tools/naming.py --write    re-record the baseline after a migration batch
    tools/naming.py --proba    show the check firing on a new name
"""
...
def zapysaty(imena: set[str]) -> None:
    r = ["# Transliterated identifiers still in the tools",
         "",
         "> **generated** — `tools/naming.py --write`. Editing it by hand only",
         "> moves the next run's diff, never the code it describes.",
         "",
         "**Generated by `tools/naming.py --write`.** This list may shrink and",
