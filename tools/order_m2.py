...
Закон М1 (`HELPERS.md`): найдешевша відповідь у наряді має бути
...
1. **Підтвердження мусить бути підрядком названого файлу.** Не «книга
   ...
   `factcheck/source-cache/ds18b20.pdf`. Переписати текст книги назад більше не
   спрацює: тексту книги в тому файлі немає.
2. **Файл названо в наряді.** Помічник не шукає документ — він його
   відкриває. Дешева модель безпечна саме тут (`HELPERS.md`); там,
...
def main() -> int:
    ...
    # Каталог хвилі: `factcheck/data/runs/wave2` перейменовано на
    # `factcheck/data/runs/wave2`. Правило заміни цього рядка **не** зачепило, і
    ...
    out = ROOT / "factcheck" / "data" / "runs" / ("m2-order-%s.yaml" % imya)
    import yaml
