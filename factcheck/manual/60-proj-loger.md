...
- **Клас:** 🟢 B — первинне похідне — першоджерело отримано, твердження випливає однозначно
- **Джерело:** Власний код розділу 60 (`if (napruga < 3.2f) { … esp_deep_sleep_start(); }`, без таймера пробудження) і власне твердження цього ж розділу «Buck-boost працює до 3.0 В»
- **Дослівно з джерела:**
  > float napruga = zmiryaty_akumulyator();      // з ключем, див. нижче
  > if (napruga < 3.2f) {
  >     ESP_LOGE(TAG, "акумулятор %.2f В — засинаємо назавжди", napruga);
