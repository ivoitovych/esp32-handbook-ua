...
- **Спосіб і дата:** Даташит IRF540 (Vishay) з кешу, 2026-08-28. Виведення, не цитата:
  поріг затвора VGS(th) = 2.0…4.0 В (VDS = VGS, ID = 250 мкА);
  опір відкритого каналу RDS(on) = 0.077 Ом заданий при VGS = 10 В.
Тобто при 3.3 В на затворі напруга лежить у смузі порога — а для примірника з порогом 4.0 В вона нижча за поріг, — і жодного значення RDS(on) паспорт для 3.3 В не дає взагалі. Частково відкритий канал має великий опір, а розсіяна потужність — це I²R.
Протилежний бік підтверджує IRLZ44 (source-cache/8a533279-irlz44.pdf): «RDS(on) specified at VGS = 4 V and 5 V» — саме тим ключ логічного рівня й відрізняється.
...
2026-08-28. Клас був E — «зовнішнього сигналу немає». Це присуд, і він хибний: паспорти обох ключів лежали в кеші з 26 серпня. Твердження книги виводиться з них прямо.
Клас derived, а не verbatim: жоден рядок паспорта не каже «гріється від 3.3 В» — це висновок із двох чисел, і обидва названо вище.
- **Прохід:** m2-20-levels-and-switches
