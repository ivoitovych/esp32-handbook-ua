...
Наряд — `factcheck/archive/NARYAD-m2-rozbir.md`. Перевірка — `intake_triage.py`.
    ...
    factcheck/tools/triage.py <агентів> <одиниць-на-агента> [каталог]
"""
...
spec = importlib.util.spec_from_file_location("nm", ROOT / "factcheck" / "tools" / "order_m2.py")
...
def main(argv: list[str]) -> int:
    ...
    vzhe = set()
    for f in Path(ROOT / "factcheck" / "work" / "triage").glob("*.yaml"):
        try:
