...
def mira_instrumenty() -> list[str]:
    ...
    return sorted(p.stem for p in repo.tool_files()
...
def mira_vorota() -> dict[str, int | str]:
    ...
    r = subprocess.run([sys.executable, "factcheck/tools/docs.py"], cwd=ROOT,
    ...
    r = subprocess.run([sys.executable, "factcheck/tools/intake.py"], cwd=ROOT,
                       capture_output=True, text=True)
...
def samoperevirka() -> int:
    ...
    # Поріг був `> 20` — число, зняте з дерева, у якому лежав тридцять
    # один документ. Зведення лишило дев'ятнадцять, і показ упав, хоч
    # нічого не зламалося: він стеріг **розмір**, а розмір не є
    # властивістю, яку ми обіцяли тримати.
    #
    # Стерегти треба те, заради чого перевірка є: що перелік не порожній
    # і що в ньому є ті документи, без яких технології немає. Це не
    # старіє від прибирання.
    probа(f"документи перелічено ({usi})", usi > 0)
    import doc_kind
    korin = {p.name for p in doc_kind.dokumenty()}
    probа("керівні документи в переліку",
