...
рядок у `factcheck/source-cache/MANIFEST.md`: URL, ім'я, розмір, хеш, дата.
    ...
    factcheck/tools/cache.py <URL> [ім'я]   завантажити й записати в маніфест
...
**Генерується** `factcheck/tools/cache.py`. Самі файли в git **не входять**: це
чужий матеріал під копірайтом, і перевидавати його ми не маємо права
(докладно — у шапці `factcheck/tools/cache.py`).
...
```sh
factcheck/tools/cache.py <URL>      завантажити й записати сюди
factcheck/tools/cache.py --check    звірити хеші наявних файлів
```
    ...
    for f in sorted(teka.glob("*.yaml")):
        ...
        for r in zap:
            if not isinstance(r, dict):
                continue
            if _fc().status_of(r) not in ("verbatim", "derived"):
                continue
