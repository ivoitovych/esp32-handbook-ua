...
міри є окремий інструмент із випадковою вибіркою — `factcheck/tools/sample.py`.
...
# `factcheck/tools/layer3.py` міг їх звірити **до** того, як їх побачить
# супровідник.
KANDYDATY = ROOT / "factcheck" / "work" / "queues" / "contest-e-candidates.yaml"
...
def tretiy_shar() -> dict[str, str]:
    """Прогнати кандидатів через `factcheck/tools/layer3.py` і повернути стан кожного.
...
# Це **звіт супровіднику**, а не наряд помічникові: тут зведено те,
# що помічники повернули. Блоків завдання з `TASK-SPEC.md` тут бути
# не має — вони пояснювали б читачеві звіту, як бути виконавцем.
...
def main() -> int:
    ...
    # потрапляє в реєстр неперевіреним». `layer3.py` вміє читати запис
