...
def odynyci(klas: str) -> list[dict]:
    ...
    Приймає і слово (`"unchecked"`), і літеру (`"F"`): у коментарі
    картки тепер СЛОВО, і літера переводиться в нього. Зворотний бік
    того рядка, який стояв тут до стиснення.
    ...
    """
    import factcheck
    # Звести обидва написання до СЛОВА: у коментарі картки тепер воно.
    # Перед стисненням тут стояв зворотний переклад, і після стиснення
    # `odynyci("A")` тихо не знаходила нічого — 8331 картка, нуль
    # збігів, і жодної помилки. Спіймала це не ця тула, а вартовий
    # «нуль одиниць — не результат» у `modality.py`.
