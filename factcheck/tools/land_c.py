...
З розбору М2 (`factcheck/work/triage/`), рід `dzherelo-ye` — там, де поле
...
def main() -> int:
    ...
    rozbir = pathlib.Path("factcheck/work/triage")
    kandydaty = []
    ...
    for fayl, zapys in sorted(posadka.items()):
        shapka = (
            ...
            f"#\n"
            f"# Посаджено `factcheck/tools/land_c.py`. Клас `C` — розписка з\n"
