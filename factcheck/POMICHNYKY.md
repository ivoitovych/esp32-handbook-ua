...
| `tools/mira_f.py` | рахує придатні записи | рахував **до** власних воріт |
| `tools/prochid.py` | кандидат «для кожного пакета» | мовчки пропускав рядок |
...
**Посадка (`tools/prochid_posadka.py`) — окремий обов'язковий крок.**
...
ті самі одиниці вдруге. Уроки самої посадки — у `ARCHITECTURE.md`,
...
| `T-17-063` | поділ реєстру відрізав застереження в наступну одиницю (див. `ARCHITECTURE.md`) |
...
`factcheck/proba-100/vybirka.json`). Кандидата немає **за побудовою**:
...
а половина роботи.** `TEMY` у `naryad_f.py` робить більше, ніж
виглядає.
