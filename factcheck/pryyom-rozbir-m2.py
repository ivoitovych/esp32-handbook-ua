...
"""Приймання наряду «розбір»: чотири умови, усі механічні.
  ...
  3. `pozyciya` і `ne-tverdzhennya` мають поле `chomu`;
...
def perevirka(p: Path) -> list[tuple[str, str]]:
    ...
    for z in zapysy:
        ...
        if rid not in RODY:
            ...
            continue
        if rid in ("pozyciya", "ne-tverdzhennya") and not str(z.get("chomu", "")).strip():
            bidy.append(("%s БЕЗ ПОЯСНЕННЯ" % rid.upper(), ident))
        if rid == "dzherelo-ye":
