...
- **Дослівно з джерела:**
  > —
- **Спосіб і дата:** Немає зовнішнього джерела для перевірки — див. нотатку.
- **Нотатка:** Твердження описує можливу конструкцію стороннього модуля (стабілізатор на платі знижує живлення до рівня логіки) — це логічна можливість, а не вимірюваний факт конкретної деталі чи стандарту, тому зовнішнього datasheet, який би це «підтверджував», не існує в принципі. Клас E.
- **Прохід:** m2-20-rivni-i-klyuchi
...
<!-- fc id:T-K14-021 sha:d4c687c0 src:kartky/k14-rivni.md:41 klas:F -->
...
<!-- fc id:T-K14-022 sha:6c223e67 src:kartky/k14-rivni.md:44 klas:F -->
...
<!-- fc id:T-K14-030 sha:a2e10d3e src:kartky/k14-rivni.md:58 klas:B -->
...
- **Джерело:** Texas Instruments, SN74HC04 Hex Inverter datasheet, розділ 6.5 «Electrical Characteristics», параметр `VOH`
- **Дослівно з джерела:**
  > Operating free-air temperature (TA)
  > PARAMETER                TEST CONDITIONS                    VCC                25°C                      -40°C to 85°C          UNIT
  >                                                               MIN       TYP         MAX       MIN        TYP    MAX
  > High-level        VI = VIH
  > VOH                                                           4.5 V              4.4      4.499                   4.4                       V
  > output voltage    or VIL                          IOH = -20 µA
- **Спосіб і дата:** PDF з кешу `sn74hc04.pdf`, завантажено `vzyaty.sh` з ti.com, pdftotext -layout, 2026-08-26
- **Нотатка:** Таблиця в datasheet не має окремого рядка рівно для VCC = 5 В (кроки виміру — 2 В, 4.5 В, 6 В), тому це клас B, а не A: при 4.5 В вихід у режимі легкого навантаження (`IOH = -20 µA`) уже 4.4–4.499 В, майже впритул до живлення; при живленні 5 В вихід так само близький до 5 В за тим самим механізмом КМОП-виходу (rail-to-rail при малому струмі). Це виразно вище за абсолютний максимум входу ESP32 (3.6 В, `m2-06-napruga-mezhi.yaml`) — висновок «усі виходи» небезпечні для прямого з'єднання підтверджується. SN74HC04 — один вентиль родини 74HC; електричні характеристики виходу однакові в межах родини (та сама технологія КМОП-виходу), тому представницьке значення для «логіки 74HC» узагалі, а не лише для однієї мікросхеми.
- **Прохід:** m2-20-rivni-i-klyuchi
