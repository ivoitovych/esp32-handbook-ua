...
more during a tool rename.
    ...
    factcheck/tools/entry_points.py --capture DIR    run everything, save output
    factcheck/tools/entry_points.py --diff A B       compare two captures
    factcheck/tools/entry_points.py --missing        what no target covers
...
`factcheck/tools/factcheck.py` from the book itself, and that is exactly why it is
verbatim.
...
`factcheck/tools/schema.py` found 18 violations, four of them mine: `arithmetic`
...
This table is checked by `factcheck/tools/schema.py`, in both directions: that the
...
**Check it only this way:**
    ...
    factcheck/tools/factcheck.py vzirets '<expression>'
...
a separate line — that is how `factcheck/tools/layer3.py` tells them from the
source's text.
...
open access (`factcheck/tools/factcheck.py blocked`), and each is closed in minutes,
...
|---|---|---|
| the sweep (`factcheck/tools/contest_e.py`) | what can be obtained | picked by hand — where the light is |
| the measure (`factcheck/tools/sample.py`) | what share of the status is false | **random**, with a seed in the order |
...
**Held by.** `factcheck/tools/leak.py` (with self-check).
...
**Held by.** `factcheck/tools/modality.py` — a report, not a gate. Its first
...
**Held by.** `factcheck/tools/cache_vs_book.py` — by sha256 of contents (a copy
...
**Held by.** `factcheck/tools/cache_vs_book.py`, both conditions, in `make check`.
...
**Held by.** `factcheck/tools/docs.py`, which asks `git ls-files` on the cache
...
**How to change it.** Edit a block, run `factcheck/tools/task_spec.py --version`,
...
> assembled from there; the gate `factcheck/tools/intake_f.py` checks that same list,
> and `factcheck/tools/docs.py` compares the two. This table shows **what to demand
...
`factcheck/tools/measure_f.py` counts `not_found` with no address as a separate
...
| `factcheck/tools/measure_f.py` | counts usable records | counted **before** its own gate |
| `factcheck/tools/sweep.py` | a candidate "for every batch" | silently skipped the line |
...
**Landing (`factcheck/tools/sweep_land.py`) is a separate mandatory step.** A pass
...
3. a verbal prohibition does not restrain an action cheaper than the work.
   ...
   violated 185 times. The same rule as a **gate** in `factcheck/tools/measure_f.py`
...
The helper puts its result in its own file, and `factcheck/tools/layer3.py <file>`
...
- **in the digest**, read through `factcheck/tools/helper_dumps.py`, which repairs
