# Files in the English zone that are still Ukrainian

> **generated** — `factcheck/tools/language.py --write`. Editing it by hand only
> moves the next run's diff, never the prose it describes.

The technology migrates to English so it can be lifted onto another
book. The book and its cards stay Ukrainian, and
letters, history and run output are frozen at their date.

This list may shrink and must never grow: `make check` fails on any
file that becomes Ukrainian, or is created Ukrainian, in the English
zone. That second half is the one that matters — the foundation
acquired two new Ukrainian documents in a single afternoon while
every check was green.

**Remaining: 76 of 85 files.** Done: 9.

## Foundation

- `factcheck/METHOD.md` — ✓ English
- `factcheck/REPORT.md` — ✓ English

## Still Ukrainian

- `factcheck/reports/SWEEP-NO-SIGNAL.md` — 95 %
- `factcheck/reports/MEASURE-UNCHECKED.md` — 91 %
- `factcheck/reports/BRIEF-LEADS.md` — 76 %
- `factcheck/reports/MEASURE-NO-SIGNAL.md` — 76 %
- `factcheck/reports/SPLIT.md` — 72 %
- `factcheck/reports/BOOK-VS-SOURCES.md` — 69 %
- `factcheck/reports/UNREACHABLE-SOURCES.md` — 67 %
- `factcheck/reports/REFUTED.md` — 67 %
- `factcheck/tools/helper_dumps.py` — 66 %
- `factcheck/tools/arithmetic.py` — 66 %
- `factcheck/tools/modality.py` — 66 %
- `factcheck/tools/measure_f.py` — 64 %
- `factcheck/reports/QUOTES.md` — 64 %
- `factcheck/tools/calques.py` — 64 %
- `factcheck/tools/sample.py` — 62 %
- `factcheck/tools/layer3.py` — 62 %
- `factcheck/tools/snapshot.py` — 61 %
- `factcheck/tools/cache_vs_book.py` — 60 %
- `factcheck/tools/docs.py` — 60 %
- `factcheck/tools/factcheck.py` — 60 %
- `factcheck/tools/intake.py` — 59 %
- `factcheck/tools/sweep_land.py` — 59 %
- `factcheck/tools/contest_e.py` — 59 %
- `factcheck/tools/layer1_units.py` — 58 %
- `factcheck/tools/work_orders.py` — 54 %
- `factcheck/tools/split_queue.py` — 54 %
- `factcheck/tools/order_m2.py` — 53 %
- `factcheck/tools/patterns_repair.py` — 53 %
- `tools/pdf-smoke.py` — 52 %
- `factcheck/tools/intake_wave3.py` — 51 %
- `factcheck/tools/sweep.py` — 50 %
- `factcheck/tools/leads.py` — 50 %
- `factcheck/tools/layer3_m2_legacy.py` — 50 %
- `factcheck/tools/leak.py` — 49 %
- `factcheck/tools/cache.py` — 48 %
- `factcheck/tools/cache_identity.py` — 48 %
- `tools/correspondence.py` — 48 %
- `factcheck/tools/work_orders_f.py` — 47 %
- `factcheck/tools/spelling.py` — 47 %
- `tools/intake_triage.py` — 46 %
- `factcheck/tools/schema.py` — 46 %
- `tools/book_index.py` — 45 %
- `factcheck/tools/refuted.py` — 44 %
- `factcheck/tools/land_c.py` — 44 %
- `factcheck/tools/intake_wave2.py` — 43 %
- `factcheck/tools/field_names.py` — 43 %
- `factcheck/tools/sweep_digest.py` — 42 %
- `tools/budgets.py` — 41 %
- `tools/struct_fields.py` — 41 %
- `tools/cross_refs.py` — 41 %
- `tools/pins.py` — 41 %
- `factcheck/tools/coverage.py` — 40 %
- `factcheck/tools/renames.py` — 40 %
- `factcheck/reports/TRACES.md` — 37 %
- `factcheck/reports/BRIEF-QUOTES.md` — 37 %
- `tools/review.py` — 37 %
- `tools/build.py` — 36 %
- `tools/linkcheck.py` — 35 %
- `factcheck/tools/layer1.py` — 35 %
- `factcheck/tools/intake_f.py` — 33 %
- `factcheck/reports/BRIEF-SAMPLE.md` — 32 %
- `factcheck/tools/doc_kind.py` — 31 %
- `tools/claims.py` — 31 %
- `factcheck/tools/language.py` — 30 %
- `factcheck/tools/task_spec.py` — 28 %
- `factcheck/tools/maintenance.py` — 28 %
- `factcheck/tools/triage.py` — 25 %
- `factcheck/tools/name_lists.py` — 25 %
- `factcheck/tools/naming.py` — 22 %
- `factcheck/tools/entry_points.py` — 19 %
- `tools/kod-stubs.py` — 14 %
- `factcheck/tools/bind_by_hash.py` — 14 %
- `factcheck/reports/RUNS.md` — 14 %
- `tools/preview.py` — 14 %
- `factcheck/tools/deslang.py` — 11 %
- `factcheck/tools/wave.py` — 6 %
