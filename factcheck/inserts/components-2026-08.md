...
<!-- fc id:T-COM-022 sha:923e96e8 src:inserts/components-2026-08.md:28 klas:E -->
...
- **Клас:** ⚪ E — поза зовнішньою звіркою — редакційне рішення, порада, рамка викладу
- **Нотатка:** Маршрут вибору («де можна» визнає, що не завжди), не факт про жоден із двох чипів.
- **Прохід:** m2-22-vkladysh-components
...
<!-- fc id:T-COM-028 sha:37cc4d8c src:inserts/components-2026-08.md:31 klas:E -->
...
- **Клас:** ⚪ E — поза зовнішньою звіркою — редакційне рішення, порада, рамка викладу
- **Нотатка:** «Беріть X» — маршрут вибору читача, не паспортний факт; сам ACS712 у списку «одразу клас C» (analog.com недосяжний із цієї мережі), але цей рядок його електричні параметри не стверджує — лише напрямок заміни.
- **Прохід:** m2-22-vkladysh-components
...
- **Нотатка:** BSS138 — типовий транзистор цієї схеми (SparkFun BOB-12009 та аналоги), тож джерело перевірено. Datasheet дає лише електричні параметри самого MOSFET (`BVDSS` 50 В, `RDS(ON)` 3.5 Ω) і жодного слова про застосування як двонапрямлений конвертер рівнів — ця схема (одна лінія, підтягнута резистором з кожного боку до свого живлення) описана в застосунковій ноті виробників плат, не в datasheet кристала. Призначення лишається редакційним поясненням типової схеми, не паспортним фактом одного чипа.
- **Прохід:** m2-22-vkladysh-components
...
- **Клас:** ⚪ E — поза зовнішньою звіркою — редакційне рішення, порада, рамка викладу
- **Нотатка:** Маршрут використання, а не технічна характеристика перетворювача: buck-boost годиться і для мережевого живлення, «автономний пристрій» — це порада, коли саме він виправданий (від батареї, де вхідна напруга гуляє по обидва боки від 3.3 В).
- **Прохід:** m2-22-vkladysh-components
...
<!-- fc id:T-COM-072 sha:ef5efd9a src:inserts/components-2026-08.md:77 klas:E -->
...
- **Клас:** ⚪ E — поза зовнішньою звіркою — редакційне рішення, порада, рамка викладу
- **Джерело:** IRLZ44, IRLZ44N2 datasheet — перевірено й не дає такого номіналу
- **Спосіб і дата:** PDF Vishay/Infineon, кеш `irlz44.pdf`, `irlz44n2.pdf`, pdftotext -layout, 2026-08-26
- **Нотатка:** Обидва datasheet логічного MOSFET дають електричні межі затвора (`VGS(th)`, ємності), але не рекомендують конкретний номінал підтягувального резистора на землю — це схемотехнічна практика (щоб затвор не «висів» у стані Hi-Z при відключеному керуючому піні), спільна для будь-якого MOSFET, а не паспорт одного чипа.
- **Прохід:** m2-22-vkladysh-components

---

<!-- fc id:T-COM-073 sha:080b6bae src:inserts/components-2026-08.md:78 klas:E -->
...
- **Клас:** ⚪ E — поза зовнішньою звіркою — редакційне рішення, порада, рамка викладу
- **Нотатка:** Той самий випадок, що й підтягування на землю: гасить дзвін і обмежує пусковий струм заряду ємності затвора, значення обирають під конкретний MOSFET і драйвер, у жодному з наявних datasheet (`irlz44`, `irlz44n2`, `bss138`) немає рекомендованого числа.
- **Прохід:** m2-22-vkladysh-components
...
<!-- fc id:T-COM-075 sha:f17c30b7 src:inserts/components-2026-08.md:80 klas:E -->
...
<!-- fc id:T-COM-081 sha:06de9e89 src:inserts/components-2026-08.md:90 klas:E -->
...
- **Клас:** ⚪ E — поза зовнішньою звіркою — редакційне рішення, порада, рамка викладу
- **Нотатка:** «Нестабільно, зникає після оновлення драйвера» — поведінкова ознака підробки на боці Windows/драйвера, не електрична характеристика CP2102 чи FT232; жоден datasheet такого не пише й не міг би — підробки не документуються виробником оригіналу. Той самий клас, що й сусідні ознаки підробок (перемаркування, менша флеш) в тому ж розділі.
- **Прохід:** m2-22-vkladysh-components
...
<!-- fc id:T-COM-085 sha:5b74e29e src:inserts/components-2026-08.md:95 klas:C -->
...
<!-- fc id:T-COM-087 sha:6aad9090 src:inserts/components-2026-08.md:95 klas:B -->
...
- **Клас:** 🟢 B — первинне похідне — першоджерело отримано, твердження випливає однозначно
- **Джерело:** Похідне від попереднього запису цього ж файлу (±0.5 °C на датчик, Maxim DS18B20 datasheet)
- **Спосіб і дата:** Логічний висновок, без нового джерела, 2026-08-26
- **Нотатка:** Пряме продовження попереднього факту: якщо поріг збігається з тією самою межею ±0.5 °C, то два справні датчики, що розійшлися на власну паспортну похибку (до 1.0 °C сумарно), потрапили б під поріг як «підробка». Висновок випливає однозначно з числа, дослівної фрази в джерелі немає — тому B, а не A.
- **Прохід:** m2-22-vkladysh-components
