...
Записів доказів: **410**. Звірено дослівно: **68**. Не знайдено: **51**. Джерело не в кеші: **30**. Нема чого звіряти: **258**.

Станом на 2026-08-26 21:45 UTC.
...
| Живлення 5 В і виводи 3.3 В на одному модулі — загальна можливість, не вимірюваний факт | `m2-20-rivni-i-klyuchi` | немає придатних уривків |
| Живлення 3.3 В без сприйняття 3.3 В як одиниці — та сама логічна можливість | `m2-20-rivni-i-klyuchi` | немає придатних уривків |
...
| «USB-роз'єм... стабілізатор... 3.3 В» — топологія плати розробки, не факт із datasheet кристала | `m2-21-zhyvlennya-06` | немає придатних уривків |
| «Сюди можна подавати 5 В (залежить від стабілізатора на платі)» — явно позначена залежність від конкретної плати | `m2-21-zhyvlennya-06` | немає придатних уривків |
...
| Релейний модуль з оптопарою — 5 В і інверсна логіка не паспортні | `m2-22-vkladysh-components` | немає придатних уривків |
| Конвертер рівнів на польових — призначення 3.3↔5 В | `m2-22-vkladysh-components` | немає придатних уривків |
| Buck-boost 3.3 В — призначення «автономний пристрій» | `m2-22-vkladysh-components` | немає придатних уривків |
...
| Buck-boost і резистори 4.7 кОм — кількість і службова примітка в BOM | `m2-23-proekty-60-62` | немає придатних уривків |
...
| Модуль реле — коло котушки на окремих 5 В, а не 3V3 (без конкретної мікросхеми) | `m2-23-proekty-60-62` | немає придатних уривків |
...
| Резистори 220 Ом і 10 кОм модуля реле й поплавка — кількість у BOM | `m2-23-proekty-60-62` | немає придатних уривків |
...
| HC-SR04 — дільник напруги 10кОм + 20кОм | `m2-28-sensory-45` | немає придатних уривків |
...
| Конденсатор 470 мкФ між 3V3 і GND поруч із модулем | `m2-31-kartka-k13` | немає URL |
| Керамічний 100 нФ біля кожної мікросхеми | `m2-31-kartka-k13` | немає URL |
| Окреме джерело від 1 А — не 500 мА, тому що платити треба за піки | `m2-31-kartka-k13` | немає URL |
| Живлення 3.3 В напряму, мимо бортовий LDO — коли він слабкий на клоне | `m2-31-kartka-k13` | немає URL |
