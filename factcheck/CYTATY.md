...
Записів доказів: **659**. Звірено дослівно: **108**. Не знайдено: **69**. Джерело не в кеші: **40**. Нема чого звіряти: **386**.

Станом на 2026-08-26 23:08 UTC.
...
| Доказ | Файл | Деталі |
|---|---|---|
| Дешевий USB-кабель — причина перезавантаження через падіння напруги | `m2-65-elektronika-05` | клас B, а джерело — не документ: «Висновок з закону Ома (U = I × R). Падіння напруги на кабелі» |
| Формула резистора для світлодіода — R = (U_живлення − U_світлодіода) / I_бажаний | `m2-65-elektronika-05` | клас A, а джерело — не документ: «Класична формула для обмеження струму світлодіода, випливає » |
| Релейні модулі для Arduino — 5-вольтові логічні входи | `m2-65-elektronika-05` | клас B, а джерело — не документ: «Типові 5-вольтові релейні модулі для Arduino та подібних пла» |
| ESP32 3.3 В часто сприймається як HIGH 5-вольтовими входами | `m2-65-elektronika-05` | клас B, а джерело — не документ: «Порогові напруги 5-вольтової логіки (стандарт TTL/CMOS 5V): » |
| Резистор як перетворювач рівня — не працює для SPI (завалює фронти) | `m2-65-elektronika-05` | клас B, а джерело — не документ: «SPI протокол вимагає крутих фронтів для синхронізації. Опір » |
| Резистор як перетворювач рівня — не працює для I²C (відкриває коло) | `m2-65-elektronika-05` | клас B, а джерело — не документ: «I²C вимагає open-drain/open-collector виходів та pull-up рез» |
| Pull-up резистор — електричний опір від сигнальної лінії до живлення | `m2-65-elektronika-05` | клас A, а джерело — не документ: «Визначення з електротехніки» |
| Спільна земля — сигнали це напруги відносно точки відліку (GND) | `m2-65-elektronika-05` | клас B, а джерело — не документ: «Базовий принцип електроніки» |
| Конденсатор 100–470 мкФ біля живлення — стабілізація напруги | `m2-65-elektronika-05` | клас B, а джерело — не документ: «Типовий скейм розділення живлення для мікроконтролерів» |
| MOSFET затвор — резистор 100–220 Ом від GPIO, захист від перегріву | `m2-65-elektronika-05` | клас B, а джерело — не документ: «Типові схеми управління MOSFET та рекомендації паспортів MOS» |
| MOSFET затвор — резистор 10 кОм від затвора до землі, утримання LOW при старті | `m2-65-elektronika-05` | клас B, а джерело — не документ: «Типовий розробник MOSFET схем та рекомендації для безпечної » |
| GPIO при старті не налаштований, лінія "висить" — небезпека вмкнення | `m2-65-elektronika-05` | клас B, а джерело — не документ: «Аналіз поведінки GPIO при старті мікроконтролера» |
| Таблиця порівняння приладів діагностики — назва, призначення, ціна | `m2-66-analizator-28` | клас B, а джерело — не документ: «Типова таблиця порівняння приладів для цифрової діагностики» |
| GND (земля) між пристроями мусить мати нульовий опір (спільна земля) | `m2-66-analizator-28` | клас B, а джерело — не документ: «Базовий принцип електроніки та сигнальних схем. Висновок з з» |
| Восьмиканальний USB-аналізатор коштує як пара плат (≈100–200 грн) | `m2-66-analizator-28` | клас B, а джерело — не документ: «Типові ціни на USB-аналізатори на онлайн-маркетплейсах (AliE» |
| UART декодування — один канал на лінію, помилка швидкості видна як сміття | `m2-66-analizator-28` | клас B, а джерело — не документ: «UART протокол: послідовна передача 8 біт за заданою швидкіст» |
| SPI діагностика — потрібні чотири канали (SCK, MOSI, MISO, CS) | `m2-66-analizator-28` | клас A, а джерело — не документ: «SPI протокол: чотирипровідний інтерфейс послідовної передачі» |
| SPI помилка: дані зчитуються не по тому фронту → видно на аналізаторі | `m2-66-analizator-28` | клас B, а джерело — не документ: «SPI має кілька режимів (CPOL, CPHA) які визначають який фрон» |
| I²C сканер — програма що перебирає адреси й друкує які відповідають | `m2-66-analizator-28` | клас B, а джерело — не документ: «Типовий утиліт для діагностики I²C шин. Багато бібліотек маю» |
| Мультиметр — вимір напруги, опору, малих струмів | `m2-66-analizator-28` | клас B, а джерело — не документ: «Базовий вимірювальний прилад, доступна у будь-якої радіоелек» |
| Осцилограф — спостереження форми сигналу та часових параметрів | `m2-66-analizator-28` | клас B, а джерело — не документ: «Професійний вимірювальний прилад для аналізу аналогових сигн» |
| Кнопка, що подає сигнал на GPIO, не працює, коли чип завис | `m2-67-proj-62` | клас B, а джерело — не документ: «базова логіка: якщо чип завис, GPIO не обробляються» |
| Linux: Доступ до серійного порту вимагає групи dialout | `m2-72-symptomy-29` | клас B, а джерело — не документ: «Linux권限 (permissions) файлів; udev rules для /dev/ttyUSB*» |
| Практична межа SPI — 1 МГц для запуску | `m2-80-shyny` | клас B, а джерело — не документ: «ESP-IDF документація; практична порада проектування» |
...
| GPIO15 часто спричиняє мовчання плати при неправильній обв'язці | `m2-63-gpio-07` | немає URL |
| Перевірка GPIO15 обв'язки коштує простого скидання плати | `m2-63-gpio-07` | немає придатних уривків |
| Напруга 1.8 В на чипі ESP32 блокує всі ознаки життя плати | `m2-63-gpio-07` | немає URL |
| На модулях з флешем 1.8 В це правильне налаштування для SDIO | `m2-63-gpio-07` | немає URL |
| GPIO12 контролює напругу живлення VDD_SDIO | `m2-63-gpio-07` | немає URL |
| Підтягувальний резистор на strapping-піні — частиста причина збоїв | `m2-63-gpio-07` | немає URL |
| GPIO6-11 на classic ESP32 підключені до флешу та недоступні | `m2-63-gpio-07` | немає URL |
| GPIO6-11 виведені на гребінку більшості плат, але це пастка | `m2-63-gpio-07` | немає придатних уривків |
| Правило categoric - GPIO6-11 на classic не існують для користувача | `m2-63-gpio-07` | немає придатних уривків |
| На WROOM-32 GPIO6-11 вільні та можуть використовуватися | `m2-63-gpio-07` | немає придатних уривків |
| WROVER модуль має вже зайнятий GPIO16 підключенням до PSRAM | `m2-63-gpio-07` | немає URL |
| S3 модулі з PSRAM мають іншу розпиновку ніж без PSRAM | `m2-63-gpio-07` | немає URL |
| У S3 и C3 немає тільки-вхідних пінів, усі повнофункціональні | `m2-63-gpio-07` | немає URL |
...
| DAC лише на classic ESP32 в лінійці | `m2-63-gpio-07` | немає URL |
| Touch сенсори є лише на classic, S2 и S3 | `m2-63-gpio-07` | немає URL |
| Аналогові модулі (ADC, DAC, Touch) фізично привязані до пінів, матриця GPIO не дія | `m2-63-gpio-07` | немає придатних уривків |
| UART0 (консоль) на пінах 1 та 3, краще його не міняти | `m2-63-gpio-07` | немає URL |
| На classic ESP32: 6 пінів флешу, 2 консолі, 6 тільки-вхідних, 5 strapping | `m2-63-gpio-07` | немає URL |
| Всього на classic залишається близько 20 повноцінних пінів | `m2-63-gpio-07` | немає придатних уривків |
| Типовий проєкт витрачає пліни швидко: I²C=2, SPI=4+, UART=2 і більше | `m2-63-gpio-07` | немає придатних уривків |
| S3 має більше пінів ніж classic ESP32 | `m2-63-gpio-07` | немає URL |
| Пліни на платі можуть мати альтернативні назви (D0, A0, SDA замість GPIO номерів) | `m2-63-gpio-07` | немає придатних уривків |
| ESP32 DevKit V1 продається в двох варіантах: 30-піновий та 38-піновий | `m2-63-gpio-07` | немає придатних уривків |
| Strapping-піни можна використовувати як виходи, але з обережністю | `m2-63-gpio-07` | немає придатних уривків |
| Вхід, кнопка чи датчик на strapping-піні спричиняє випадкові збої при перезавантаженні | `m2-63-gpio-07` | немає придатних уривків |
| Кожен strapping-пін має внутрішній резистор саме такого номіналу | `m2-63-gpio-07` | немає придатних уривків |
| Кнопка BOOT як перемикач на землю конкурує з внутрішнім підтяганням | `m2-63-gpio-07` | немає придатних уривків |
| Таблиця завантаження: GPIO0/GPIO15 комбінації для режимів | `m2-63-gpio-07` | немає придатних уривків |
| На classic та S3 немає комбінації для входу в download режим без USB | `m2-63-gpio-07` | немає URL |
| Правило для strapping: використовуй як виходи, ті що не використовуються разу до завантаження | `m2-63-gpio-07` | немає придатних уривків |
| ESP32 — не один чип, а сімейство, що росте років десять | `m2-64-chipy-02` | немає URL |
| Різні чипи в сімействі ESP32 вимагають різного коду та плат | `m2-64-chipy-02` | немає придатних уривків |
| ESP32 classic та S3 — два основні варіанти, про які книга розповідає детально | `m2-64-chipy-02` | немає придатних уривків |
| ESP32-C3 як дешевий мінімалістичний варіант без надмірностей | `m2-64-chipy-02` | немає URL |
| S2, C6, H2 — спеціалізовані варіанти для конкретних завдань | `m2-64-chipy-02` | немає придатних уривків |
| C2, C5, C61, H4, P4 — рядкові варіанти, що рідко зустрічаються | `m2-64-chipy-02` | немає придатних уривків |
