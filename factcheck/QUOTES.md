...
Записів доказів: **1360**. Звірено дослівно: **524**. Не знайдено: **104**. Джерело не в кеші: **30**. Нема чого звіряти: **679**.

Станом на 2026-08-28 18:58 UTC.
...
| T-02-103: Усі налаштування, зроблені через `menuconfig`, повертаються до типових. | `klas-f-02-chipy` | 1 з 1 рядків: «idf.py set-target will clear the build directory and re-generate the s…» |
...
| T-11-112: `set-target` стирає `sdkconfig`; у git кладеться `sdkconfig.defaults`. | `klas-f-11-idf` | 1 з 1 рядків: «idf.py set-target will clear the build directory and re-generate the s…» |
...
| T-07-136: Strapping-піни краще використовувати як виходи й лишати вільними | `prochid-07-gpio` | 1 з 1 рядків: «Strapping pin: GPIO0, GPIO2, GPIO5, GPIO12 (MTDI), and GPIO15 (MTDO) a…» |
| T-07-053: На classic і S3 такої комбінації немає — | `prochid-07-gpio` | 1 з 1 рядків: «Strapping pin: GPIO0, GPIO2, GPIO5, GPIO12 (MTDI), and GPIO15 (MTDO) a…» |
...
| T-31-085: Усі програмні таймери виконуються в **одній** службовій задачі. | `prochid-31-freertos` | 1 з 1 рядків: «FreeRTOS Timer Task (Tmr Svc)…» |
| T-33-034: Це основний спосіб міряти час: переповнення не станеться | `prochid-33-peryferiya-kod` | 1 з 1 рядків: «The maximum count value depends on the bit width of the hardware timer…» |
...
| T-C-092: `/dev/ttyUSB*` — зовнішній міст. | `prochid-c-komandy` | 1 з 1 рядків: «For Linux users, if the port name is /dev/ttyUSB0…» |
| T-E-125: WS2812 / SK6812 · Як → **RMT**, не | `prochid-e-interfeysy` | 1 з 1 рядків: «rmt/led_strip demonstrates how to use the RMT peripheral to drive a WS…» |
