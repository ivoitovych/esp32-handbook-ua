...
**Формат колонок — той самий, що в `dzherela-kesh/MANIFEST.md`** (`tools/cache.py`),
щоб рядки можна було злити механічно.

## Чому окремий файл, а не спільний маніфест

`tools/cache.py` **перегенеровує** `MANIFEST.md` із вмісту свого каталогу.
...
Якщо `cache.py` навчиться **доповнювати** маніфест замість переписувати —
...
Перевірка на цей рід — `tools/cache_identity.py`. Вона не ворота:
