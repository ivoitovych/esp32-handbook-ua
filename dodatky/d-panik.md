...
Читається так само: біт виставлений — пін був високим. Тобто на C3
`boot:0xc` — обидва високі, звичайний старт; `boot:0x4` — `GPIO9`
низький при високому `GPIO8`, тобто коректний вхід у download mode.

А `boot:0x0` на C3 варто впізнавати окремо: обидва піни низькі — це та
сама **недійсна комбінація**, про яку попереджає розділ 07. Поведінка
після неї непередбачувана, і шукати причину треба не в прошивці, а в
тому, що тримає `GPIO8` унизу.
