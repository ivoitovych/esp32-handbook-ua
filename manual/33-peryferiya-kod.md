...
2 мс — інший. Для 16-розрядної роздільності на 50 Гц це відповідає
...
led_strip_config_t scfg = { .strip_gpio_num = 8, .max_leds = 30 };
...
дільник **сам розряджає акумулятор**. Два резистори по 100 кОм постійно
беруть десятки мікроампер: для пристрою, що споживає уві сні одиниці
мікроампер, це головне джерело розряду.
