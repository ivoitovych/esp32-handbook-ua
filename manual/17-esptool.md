...
під v4. Дослівно на v5 вони не працюють, і навпаки. Помилка при цьому
виглядає як «невідома команда», що збиває з пантелику: команда відома,
просто пишеться інакше.
...
На [[S3]] [[C3]] і новіших бутлоадер іде не на `0x1000`, а на `0x0`;
решта адрес та сама. Скопіювати цей приклад дослівно на S3 означає
покласти бутлоадер у порожнє місце — і `esptool` не поскаржиться
(розділ 16).
...
[[classic]] Адреса `0x1000` тут — знову classic; для S3, C3 і новіших
на її місце йде `0x0`. Адреси всередині `merge-bin` мають бути ті самі,
якими прошивали б окремо.
